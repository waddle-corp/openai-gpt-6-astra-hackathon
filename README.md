...
4. Hover a shopper card to highlight its workers. Use the bottom controls to restart or scrub the simulation, or start the guided demo with the circular REC control. These controls only change presentation; they never start an investigation, approve an improvement or transfer money. Output details use the current event projection, so later rewards do not appear early.
...
### One-minute recording mode

Click the circular REC control in the bottom bar to start a 60-second guided sample demo. It opens the shopper report, computer-use view, findings, improvement and rewards automatically, with short factory-overview transitions. Modal pages advance automatically; the final completed sample outcome stays on screen at `01:00` instead of looping. Click the control to pause/resume, or use the pause button in an open guided modal. Closing a guided modal exits the tour; the restart control replays it from the beginning.

The tour uses the bundled delivery story independently of new submissions and does not change stored feedback. Its presentation clock continues while guided modals are open; normal inspection modals still pause the ambient simulation. `lib/demo-tour.ts` defines the deterministic chapter and sample-event timing. Use a screen recorder to capture playback; the REC-style control starts the presentation and does not capture or export a video file.
